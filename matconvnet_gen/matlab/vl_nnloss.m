...
if(numel(size(X)) == 5)
    dim5 = true;
    X = reshape(squeeze(X),[1 1 size(X,4) size(X,5)]);
end
...
if(size(c,1) == 2)
    classWeights = c(2,:);
    classWeights = reshape(classWeights, [1 1 1 inputSize(4)]);

    c(2,:) = [];
else
    classWeights = ones(1,1,1,inputSize(4),'single');
end

% Form 1: C has one label per image. In this case, get C in form 2 or
% form 3.
if ((~strcmp(opts.loss,'l1')) && (~strcmp(opts.loss,'l1w')) && (~strcmp(opts.loss,'l2')) && (~strcmp(opts.loss,'l2w')))  
  c = gather(c) ;
...
switch lower(opts.loss)
  ...
  case {'binaryerror', 'binarylog', 'logistic', 'hinge', 'l2','l2w', 'l1','l1w'}
    ...
    case 'topkerror'
      ...
      case 'weightederror'
          ...
          for ii = 1:numel(c)
              ...
              currchat = chat(ii);
              if(currc == currchat)
                  correct(find(uL == currc)) = correct(find(uL == currc)) + 1;
              end
          end
          t = 1 - mean(correct ./ uW);
          t = t * batchSize;
...
%       t = t .* classWeights;
    ...
    case 'hinge'
      t = max(0, 1 - c.*X) ;
    case 'l2'
      t = abs(sum(sum(sum((c - X) .^ 2 ,1),2),3)) ;
      % error
    case 'l1'
%         featNumel = numel(c) / size(c,4);
%         featNumel = numel(c) / size(c,4) / 100000;
%       t = abs(sum(sum(sum((c - X),1),2),3)) / featNumel;
      t = abs(sum(sum(sum((c - X),1),2),3)) ;
    case 'l2w'
      l2temp = (c - X) .^ 2 ;
      l2temp = l2temp .* instanceWeights;
      t = abs(sum(sum(sum(l2temp,1),2),3)) ;
    case 'l1w'
      t = abs(sum(sum(sum((c - X),1),2),3)) ;
    case 'contrastive'
      t = c .* (X .^ 2) + (1 - c) .* ( max (0, opts.margin - X) .^2 ) ;
      fprintf(' D/d/E : %.2f / %.2f / %.2f, ',mean(X),mean(X(c(:)==1))-mean(X(c(:)==0)),mean(t(:)));
  end
  if (~strcmp(opts.loss,'l1')) && (~strcmp(opts.loss,'l2')) && (~strcmp(opts.loss,'l1w')) && (~strcmp(opts.loss,'l2w'))
      if ~isempty(instanceWeights)
          Y = instanceWeights(:)' * t(:) ;
      else
          Y = sum(t(:));
  ...
  switch lower(opts.loss)
    case {'classerror', 'topkerror', 'weightederror'}
      Y = zerosLike(X) ;
    case 'log'
      Y = zerosLike(X) ;
    ...
    case 'softmaxlog'
          ...
          % Class Weights 
          Y = bsxfun(@times, classWeights, Y);
          if(dim5)
              Y = reshape(Y,[1 size(Y,1) size(Y,2) size(Y,3) size(Y,4)]);
          end
    case 'mhinge'
      Y = zerosLike(X) ;
      Y(ci) = - dzdy .* (X(ci) < 1) ;
    ...
    case 'l2'
        featNumel = numel(c) / size(c,4);
        Y = -dzdy .* (c - X) / featNumel;
    case 'l1'
        featNumel = numel(c) / size(c,4);
        Y = -dzdy .* sign(c - X) / featNumel;
    case 'l2w'
        featNumel = numel(c) / size(c,4);
        Y = -dzdy .* (c - X) / featNumel;
    case 'l1w'
        ...
        Y = -dzdy .* sign(c - X) / featNumel;
    case 'contrastive'
        Y = dzdy .* ( 2 .* c .* X + 2 .* (c - 1) .* (opts.margin - X) .* (X < opts.margin) ) ;
        if(dim5)
            Y = reshape(Y,[1 size(Y,1) size(Y,2) size(Y,3) size(Y,4)]);
        end
  end
